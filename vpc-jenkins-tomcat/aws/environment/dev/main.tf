provider "aws" {
  access_key = var.AWS_ACCESS_KEY
  secret_key = var.AWS_SECRET_KEY
  region     = "us-east-1"
}

#####
# Vpc
#####

module "vpc" {
  source = "../../modules/aws-vpc"

  vpc-location                 = "Virginia"
  namespace                    = "devops"
  name                         = "vpc"
  stage                        = "cicd"
  map_public_ip_on_launch      = "true"
  total-nat-gateway-required   = "1"
  create_database_subnet_group = "false"
  vpc-cidr                     = "10.20.0.0/16"
  vpc-public-subnet-cidr       = ["10.20.1.0/24", "10.20.2.0/24"]
  vpc-private-subnet-cidr      = ["10.20.4.0/24", "10.20.5.0/24"]
  vpc-database_subnets-cidr    = ["10.20.7.0/24", "10.20.8.0/24"]
}


module "sg1" {
  source              = "../../modules/aws-sg-cidr"
  namespace           = "devops"
  stage               = "dev"
  name                = "Jenkins"
  tcp_ports           = "22,80,443"
  cidrs               = ["0.0.0.0/0"]
  security_group_name = "Jenkins"
  vpc_id              = module.vpc.vpc-id
}

module "sg2" {
  source                  = "../../modules/aws-sg-ref-v2"
  namespace               = "devops"
  stage                   = "dev"
  name                    = "Jenkins-Ref"
  tcp_ports               = "22,80,443"
  ref_security_groups_ids = [module.sg1.aws_security_group_default, module.sg1.aws_security_group_default, module.sg1.aws_security_group_default]
  security_group_name     = "Jenkins-Ref"
  vpc_id                  = module.vpc.vpc-id
}


module "jenkins-eip" {
  source   = "../../modules/eip/ec2-eip"
  name     = "jenkins"
  instance = module.ec2-jenkins.id[0]
}

module "ec2-jenkins" {
  source                      = "../../modules/aws-ec2"
  namespace                   = "devops"
  stage                       = "dev"
  name                        = "jenkins"
  key_name                    = "jenkins-demo"
  public_key                  = file("../../modules/secrets/jenkins-demo.pub")
  user_data                   = file("../../modules/aws-ec2/user-data/jenkins/user-data.sh")
  instance_count              = 1
  ami                         = "ami-0fc61db8544a617ed"
  instance_type               = "t3a.medium"
  associate_public_ip_address = "true"
  root_volume_size            = 40
  subnet_ids                  = module.vpc.public-subnet-ids
  vpc_security_group_ids      = [module.sg1.aws_security_group_default]

}

module "tomcat-eip" {
  source   = "../../modules/eip/ec2-eip"
  name     = "tomcat"
  instance = module.ec2-tomcat.id[0]
}

module "ec2-tomcat" {
  source                      = "../../modules/aws-ec2"
  namespace                   = "devops"
  stage                       = "dev"
  name                        = "tomcat"
  key_name                    = "tomcat-demo"
  public_key                  = file("../../modules/secrets/tomcat.pub")
  user_data                   = file("../../modules/aws-ec2/user-data/tomcat/user-data.sh")
  instance_count              = 1
  ami                         = "ami-0fc61db8544a617ed"
  instance_type               = "t3a.medium"
  associate_public_ip_address = "true"
  root_volume_size            = 20
  subnet_ids                  = module.vpc.public-subnet-ids
  vpc_security_group_ids      = [module.sg1.aws_security_group_default]

}
